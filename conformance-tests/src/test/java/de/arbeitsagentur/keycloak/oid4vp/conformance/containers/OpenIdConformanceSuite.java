 ...
 */
package de.arbeitsagentur.keycloak.oid4vp.conformance.containers;
...
public final class OpenIdConformanceSuite implements AutoCloseable {
    ...
    private OpenIdConformanceSuite(
            Network network,
    ...
    private static OpenIdConformanceSuite start() {
        ...
        try {
            ...
            OpenIdConformanceSuite suite =
                    new OpenIdConformanceSuite(network, mongo, server, nginx, baseUri, sslContext);
    ...
    public URI externalUri(URI internalUri) {
        if (INTERNAL_BASE_URI.getHost().equals(internalUri.getHost())) {
            return baseUri.resolve(internalUri.getRawPath()
                    + (internalUri.getRawQuery() != null ? "?" + internalUri.getRawQuery() : ""));
        }
        return internalUri;
    }
    ...
    private static X509Certificate nginxCertificate(GenericContainer<?> nginx) {
        try {
            return nginx.copyFileFromContainer(NGINX_CERTIFICATE_PATH, input ->
                    (X509Certificate) CertificateFactory.getInstance("X.509").generateCertificate(input));
        } catch (Exception e) {
            throw new RuntimeException(
                    "Failed to read the conformance suite TLS certificate from " + NGINX_CERTIFICATE_PATH, e);
        }
    }
