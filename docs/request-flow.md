...
The `state` entry is the liveness anchor: while it exists the flow is live, and `Oid4vpDirectPostService.completeAuth` removes it (`removeRequestContext`) after a successful callback, which blocks replay. An attempt the wallet declined is removed too, once the End-User has been handed back to the login page at `/wallet-error` (`consumeWalletError`), so an attempt nobody is waiting for any more stops serving its request object and stops accepting a presentation for the rest of the login timeout.
