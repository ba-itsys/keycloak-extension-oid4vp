...
5. **Error handling** — if the wallet sent an OAuth error (for example `access_denied`), records it server-side and answers with HTTP 200, the `error`, the optional `error_description`, and the `redirect_uri` the wallet must follow to hand the End-User back to `/failed`
...
Both ways a presentation can end travel the back channel, so neither moves the browser by itself. The response the wallet gets therefore carries a `redirect_uri` alongside the error, which OID4VP 1.0 §8.2 permits for Error Responses, and cross-device flows publish the same URL on the SSE stream because there the wallet is on another device. The URL points at `/failed`, which reads the failure from the server-side record (never from its query parameters), drops the attempt's request context, and returns the End-User to the login page:
