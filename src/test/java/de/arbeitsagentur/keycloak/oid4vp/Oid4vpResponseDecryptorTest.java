...
class Oid4vpResponseDecryptorTest {
    ...
    void decryptFull_validJwe_returnsVpTokenAndNonce() throws Exception {
        ...
        Oid4vpResponseDecryptor.DecryptedResponse result = decryptor.decryptFull(jwe, encryptionKey.toJSONString());
    ...
    void tryPreDecrypt_noKid_returnsEmpty() throws Exception {
        ...
        Oid4vpResponseDecryptor.PreDecryptionResult result = decryptor.tryPreDecrypt(jwe.serialize(), null, null);

        assertThat(result).isEqualTo(Oid4vpResponseDecryptor.PreDecryptionResult.EMPTY);
    }

    @Test
    void tryPreDecrypt_invalidJwe_returnsEmpty() {
        Oid4vpResponseDecryptor.PreDecryptionResult result = decryptor.tryPreDecrypt("not-a-jwe", null, null);
