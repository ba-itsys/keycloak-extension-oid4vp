...
exec docker run --rm -i \
  ...
  -e LOAD_SD_JWT_VCT="${LOAD_SD_JWT_VCT:-urn:eudi:pid:1}" \
  ...
  -e LOAD_QR_SCAN_DELAY_MS="${LOAD_QR_SCAN_DELAY_MS:-750}" \
  -e LOAD_WALLET_APPROVAL_DELAY_MS="${LOAD_WALLET_APPROVAL_DELAY_MS:-500}" \
  -e LOAD_POST_APPROVAL_BROWSER_DELAY_MS="${LOAD_POST_APPROVAL_BROWSER_DELAY_MS:-250}" \
  -e LOAD_FIRST_BROKER_SUBMIT_DELAY_MS="${LOAD_FIRST_BROKER_SUBMIT_DELAY_MS:-400}" \
