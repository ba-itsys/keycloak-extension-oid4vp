...
/**
 * One DCQL {@code credential_sets} entry: alternative combinations of credential ids that satisfy
 * the verifier, in preference order. The wallet has to satisfy one option of every required set.
 *
 * <p>Configured verbatim in DCQL syntax on the identity provider, so the admin writes what the
 ...
 * @param required whether the wallet must satisfy one of the options; DCQL defaults this to true
 * @param purpose optional description shown to the user by the wallet
...
public record CredentialSet(List<List<String>> options, boolean required, String purpose) {
    ...
    /**
     * Parses the configured DCQL {@code credential_sets} value. Rejects everything the DCQL schema
     * does not allow, so a misconfiguration surfaces as a config error instead of as an opaque
     * wallet-side rejection.
     *
     * @throws IllegalArgumentException with an admin-readable message describing the first problem
     */
    ...
    private static CredentialSet parseEntry(JsonNode entry) {
        ...
        // An explicit null reads like an absent member, which is what a serialized credential set
        // carries for a purpose that was never configured.
    ...
    /** Every credential id this set references, in option order. */
