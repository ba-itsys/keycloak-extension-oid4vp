...
class Oid4vpCrossDeviceSseServiceTest {
    ...
    void subscribe_keepsConnectionOpenAndSendsPingBeforeTimeout() throws Exception {
        ...
        verify(sessionFactory, timeout(1000).atLeastOnce()).create();
        verify(sse.builder(), timeout(2000).atLeastOnce()).name("ping");
        verify(sse.builder(), timeout(2000).atLeastOnce()).data(String.class, "{}");
        verify(sink, timeout(2000).atLeastOnce()).send(any(OutboundSseEvent.class));
